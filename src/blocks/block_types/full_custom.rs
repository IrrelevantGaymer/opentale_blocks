...
/// plus a custom texture and model that cannot cull 
/// or be culled by other faces
pub struct FullCustom {
    /// The block's technical name
    pub(crate) name: &'static str,
    /// If no texture is defined, the default texture (a debug texture) will be used.
    pub(crate) texture: FullCustomTexture,
...
impl FullCustom {
    pub const fn with_texture(mut self, texture: &'static str) -> Self {
        self.texture = FullCustomTexture::Single(Some(texture));
        self
    }
    ...
    pub const fn with_textures(
        ...
        self.texture = FullCustomTexture::Multiple(textures.const_into());
        self
    } 
...
impl const Buildable for FullCustom {
    fn new_with_name(name: &'static str) -> Self {
        FullCustom {
            name,
            texture: FullCustomTexture::Single(None),
...
impl HasBuildVariants for FullCustom {
    type Variants = ();
}

impl BlockType for FullCustom {
...
///
/// ```rust
/// # use opentale_blocks::blocks::{block::Block, block_types::full_custom::{self, FullCustom}};
///
/// const BLOCK: FullCustom = Block::new_full_custom("block")
...
/// ---
///
/// To be able to set only some paths, see [`with_full_custom_paths!`] instead.
pub struct Paths {
    pub up: &'static str,
...
///
/// ```rust
/// # use opentale_blocks::{with_full_custom_paths, blocks::{block::Block, block_types::full_custom::{self, FullCustom}}};
///
/// const BLOCK: FullCustom = Block::new_full_custom("block")
///     .with_textures(with_full_custom_paths! {
///         up: "top.gif",
///         north: "north.gif",
///         south: "south.gif",
///         custom: "custom.gif",
...
/// see [`Paths`] instead.
#[macro_export]
macro_rules! with_full_custom_paths {
    {$($face:ident : $path:expr),* $(,)?} => {
        {
            let mut paths = $crate::blocks::block_types::full_custom::PathsInternal {
                up: None,
                north: None,
                west: None,
                east: None,
